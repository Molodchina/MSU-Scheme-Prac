...
(define (reply user-response history)
      (case (random (if (has-keywords? user-response) -1 0) (if (vector-empty? history) 2 3)) ; с равной вероятностью выбирается один из трех способов построения ответа
          ((-1) (keyword-answer user-response history)) ; 4й способ
          ((0) (hedge-answer user-response history))  ; 1й способ
          ((1) (qualifier-answer user-response history)) ; 2й способ
          ((2) (history-answer user-response history)) ; 3й способ       
       )
)

; 1й способ генерации ответной реплики -- случайный выбор одной из заготовленных фраз, не связанных с репликой пользователя
(define (hedge-answer _ __)
...
(define (qualifier-answer user-response _)
...
(define (history-answer _ history)
...
(define (keyword-answer user-response _)
...
; Создание простых интерфейсов взаимодействия со стратегиями генерации реплик
(define (create-strategy checker weight response-fn)
  (vector checker weight response-fn))
...
(define (strategy-response-fn strategy)
  (vector-ref strategy 2))

; Создаем стратегии генерации реплик
(define hedge-strategy 
  (create-strategy (lambda (user-response history) #t) 1 hedge-answer))

(define qualifier-strategy 
  (create-strategy (lambda (user-response history) #t) 2 qualifier-answer))

(define history-strategy 
  (create-strategy (lambda (user-response history) (not (vector-empty? history))) 3 history-answer))

(define keyword-strategy 
  (create-strategy (lambda (user-response history) (has-keywords? user-response)) 5 keyword-answer))

; Собираем стратегии генерации реплик в единый вектор
(define strategies (vector hedge-strategy qualifier-strategy history-strategy keyword-strategy))

; Выбор случайно стратегии на основе её веса
(define (pick-random-vector-with-weight vctr weight-selector)
  (let ((total (vector-foldl (lambda (idx res elem)
                               (+ (weight-selector elem) res))
                             0
                             vctr)))
    (if (> total 0)
        (let* ([pick (random total)]
               [result (find-strategy vctr pick 0)])
          (strategy-response-fn result))
        '())))

; Функция поиска стратегии
(define (find-strategy vctr target acc)
  (call/cc
   (lambda (return)
     (vector-foldl (lambda (idx remaining-target elem)
                     (let ((weight (strategy-weight elem)))
                       (cond
...
; Функция для выбора ответных реплик на основе одинаковых векторов для стратегий генерации ответных реплик
(define (reply-v2 user-response history)
  (define applicable-strategies
    (vector-filter (lambda (s) 
                     (let ((checker (strategy-checker s)))
                       (checker user-response history)))
                   strategies))
  (if (vector-empty? applicable-strategies)
      '("No applicable strategy found.")
      (let ((selected-strategy (pick-random-vector-with-weight 
                                 applicable-strategies 
                                 strategy-weight)))
        (selected-strategy user-response history))))
