...
; 4й способ генерации ответной реплики -- использование ключевых слов
(define (keyword-answer user-response)
  (let* ((keywords (extract-keywords user-response))
         (chosen-keyword (pick-random-vector keywords))
         (templates (get-templates chosen-keyword)))
    (let ((chosen-template (pick-random-vector templates)))
      (replace-star chosen-template chosen-keyword))))

; извлечение ключевых слов из реплики пользователя
(define (extract-keywords user-response)
  (let ((all-keywords (vector-map (lambda (group) (vector-ref group 0)) keywords-structure)))
    (filter (lambda (word) (member word all-keywords)) user-response)))

; получение шаблонов для выбранного ключевого слова
(define (get-templates keyword)
  (let ((templates (vector-map (lambda (group)
                                 (if (member keyword (vector-ref group 0))
                                     (vector-ref group 1)
                                     '()))
                               keywords-structure)))
    (apply append templates)))

; замена * на ключевое слово в шаблоне
...
; проверка наличия ключевых слов в реплике пользователя
(define (has-keywords? user-response)
  (not (vector-empty? (extract-keywords user-response))))
